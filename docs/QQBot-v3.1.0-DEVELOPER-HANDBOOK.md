...
**语言跟着「绑定姓名」走**，不做显式设置项：`qqbot.domain.names.preferred_language(name)`
——姓名含汉字 → 中文，不含汉字 → 英文（留学生）；未绑定回落中文。

| 层 | 行为 |
| --- | --- |
| 回执 | client 按请求用户姓名二选一：`self.presenters`（中文）/ `self.presenters_en`（英文）。`QQPresenter(config, lang=...)` 语言在**构造时绑定**，`render(result)` 签名不变、默认中文输出与旧版逐字节一致 |
| 命令 | `EN_COMMAND_ALIASES`（`interfaces/qq/parser.py`）：`book/reserve`、`cancel`、`my`、`my reservations`、`free`、`schedule`、`query`、`bind/register`。**参数语法本身语言中立**（房间 303、时段 21-22.5、偏移 +1 不区分语言），故别名只翻译「动词」并映射到同一批中文规范 action，业务分支零改动；大小写不敏感，且要求别名后是空白/数字/结尾（`booking` 不会被读成 `book` + `ing`）。admin（`#`）不支持别名 |
| 查询图片 | ⏳ **尚未英文化**（第二阶段）。当前一律中文渲染；英文用户拿到的图片仍是中文，文字回执已英文。实施时必须给 `ImageCache` 的 range key 加语言，否则中英用户会互相命中对方的缓存图 |
| 管理员手输的「用途」（周常/锁定 `purpose`） | 自由文本，原样显示，不翻译 |

注意：回执与图片都发在**群里**，因此英文用户查询时全群都会看到英文内容——这是
「无法私聊回执」的必然结果，属预期行为（见产品决策 2026-09-17）。

**范围**：本阶段只做「回执 + 英文命令别名」；查询图片的英文化（模板文案、站点/房间英文名、
缓存键加语言、预渲染策略）留待第二阶段。

姓名缩写（时间轴块内／占用列表）走 `names.short_display_name`：中文沿用「末 4 字」
既有约定，**外文名保留全名**——此前的 `label[-4:]` 会把 `John Smith` 截成 `mith`。
该缩写只取决于姓名本身，与查看者语言无关。
...
| 指令 | 语义 | 备注 |
| --- | --- | --- |
| `/绑定 姓名 学号` | 绑定或更新当前 QQ 身份对应用户 | 姓名须 1–24 个字符（中文或英文姓名，见 19.1） |
...
| --- | --- |
| `id` PK | 系统用户 UUID |
| `display_name` | 用户姓名（中文或英文，1–24 字符，归一化后入库，见 19.1） |
| `student_id` | 学号，可空 |
...
- 1–24 个字符；
- 只允许：汉字（`\u4e00-\u9fff`）、拉丁字母（`A-Za-z`），以及姓名常见符号 `· ・ . - ' ’` 和空格；
- 归一化规则：折叠连续空白；**纯汉字**姓名去掉空格（`张 三` → `张三`），含非汉字则保留单词间的单个空格（`John  Smith` → `John Smith`）；
- 其他文字系统（西里尔 / 阿拉伯 / 韩文）与 emoji 仍拒绝——图片字体（Noto Sans SC）无对应字形，放开只会得到豆腐块（2026-09-17 决策：先支持中文 + 拉丁）。
