...
时间轴 PNG 采用「柔和渐变扁平化」（Soft Flat）设计语言：

- **布局**：左列日期 + 房间名，右侧按小时/半小时网格的时间轴；顶部渐变横幅带站点名与统计角标，下方图例区分预约（蓝）、周常（橙）、锁定（紫）、空闲（绿）；
- **按日期交替底色**：同一天的所有房间行共享同一底色，相邻日期轻微错开（`build_timeline_view` 输出 `day_alt` 字段）；
- **块内文字**：内容名固定 13px、时间 11px 固定在块底部左下角；名字过长时**允许换行**，渲染器 `_fit_row_heights` 会把同行所有块统一到最高块高度（底部对齐），整行随之变高，不压缩字号、不出现省略号；
- **类型不写文字前缀**：预约/周常/锁定由块颜色 + 图例表达，块内不重复「周常：」等前缀；
- **昼夜主题**：`current_theme()` 按上海时区判定，19:00～次日 07:00 使用深色主题，其余时间浅色主题；模板用 CSS 变量（`body.theme-light/dark`）切换整套配色；
- **中文字体内联**：模板目录 `templates/fonts/` 下的 woff2 由渲染器读为 data URI 注入 `@font-face`（`{{ font_400 }}` / `{{ font_700 }}`），无系统字体的服务器也能渲染中文；字体随包发布（`pyproject.toml` package-data 已包含）。
